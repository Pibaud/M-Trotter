    ...
    try {
        const result = await db.query(
            `SELECT 
                ...
                p.id_lieu, 
                p.id_avis,
                COALESCE(SUM(g.vote_type * (LOG(COALESCE(u.fiabilite, 0) + 10))), 0) AS weighted_vote_score,
                COALESCE(gv.vote_type, 0) AS user_has_voted
             FROM photos p
             ...
             WHERE p.id_lieu = $1
             GROUP BY p.id_photo, p.id_lieu, p.id_avis, gv.vote_type
